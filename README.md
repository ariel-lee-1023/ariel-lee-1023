...
I never liked being taught. From early on I read on my own instead, and that is where most of my hours went — philosophy, history, literature, physics. None of it was picked for what it would be worth later. It has turned out to be worth something, and that is luck rather than foresight; I would rather say so than assemble a story about having seen this coming.

The distinction I use for what I ended up with is this. Hardware knowledge is the study of the unyielding boundary conditions of the physical world. It is knowledge of what resists us: the limits of a lens, the friction of a tire, the breaking point of a material, the constraints of a body. Software knowledge is the study of rules, sequences, and forms of life. It is knowledge of what gets constructed out of instruction, logic, and human agreement: the syntax of a coding language, the strategy of a trial, the architecture of a commercial contract. The software faculty teaches the patterns we lay over the world; the hardware faculty teaches the world that pushes back. Almost everything I have spent time on is software knowledge, and this is the period in which software knowledge is what software gets built out of. Physics is the exception on that list, and I keep at it for exactly that reason: it is the one thing I read that does not care whether the argument is elegant.
...
I build these one at a time, and no two of them share a core. What a philosopher refuses to do is not the same kind of thing as what a tribunal will not accept as evidence. I could write something general enough to cover both, but whatever survived the generalization would be too thin to lift anything — a diagram of reasoning rather than a way of reasoning about something. So the work stays divided, and it divides further the closer I look. Demand behaves like this too: a person eats what a person eats, but there are ten thousand cherry pies, each one there because somebody wanted a slightly different thing from a pie. A single general engine for judgment would not be a more ambitious version of this. It would be a decision not to look at how fine the divisions are.
...
None of this is settled yet. Nobody has decided who owns a method taken out of a book, or what it means to have rebuilt a living person's way of thinking without asking them. New technology is prospector's country: the seams are open because the rules have not arrived, and the rules do arrive. I am training to be one of the people who writes them, and I am staking claims in the meantime. The only answer I have to that is to keep the provenance files as though the rules were already in force — to record what each module was built from, and what it is not licensed to reproduce, before anyone requires it of me.
...
[`legal-reasoning-advisor`](https://github.com/ariel-lee-1023/legal-reasoning-advisor) is for the stage where the dispute is still a shapeless pile and everything in the file looks equally important. Forty pages of briefing with no way to tell which paragraph is load-bearing. A sentence that reads as a finding of fact and is doing three jobs at once — a category asserted, a relation assumed, a fact never proved. A case that can be argued either way and no account of why. It sorts an argument into the moves it is actually made of — what happened, what it counts as, what a decided case governs, what the law ought to do — and tests each by its own standard rather than by whether the paragraph reads well; it says which joint the conclusion actually hangs on, rarely the joint the brief spent its length on; and where the categories and the cases have run out it says the question is open and what would have to be argued to close it, rather than manufacturing a rule over the gap. I use it before drafting, not after.
[`Cognitive-Neuroscience-Expert`](https://github.com/ariel-lee-1023/Cognitive-Neuroscience-Expert) is for the claim about the brain that I cannot tell is settled or only well repeated. A region named as the seat of a function, when what was measured was the difference between two conditions. Two mechanisms that predict the same activation map or the same ERP component, and a design that was never built to tell them apart. A popular version of a result, and the question of whether anything is under it. It keeps the levels separate the way the field does — neuron, circuit, network, behavior — and says which one the evidence actually touches; it names the experiment where the rival accounts come apart; and where the literature does not reach that far, it says so and says what would have to be read or run to close the gap, rather than handing me something plausible.
